...
# -----------------------------------
OPENAI_API_KEY = "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"
PINECONE_API_KEY = "pcsk_4VzjMi_EmQ3tzSXMMXuZJrtHxZQ62yAfavUuVHXVUksibJr7z7jEYtgejxAVJbLU1w4iKj"
...
try:
    ...
    logger.info(
        f"Initialized Pinecone index '{INDEX_NAME}' in environment '{PINECONE_ENV}'")
    ...
    try:
        ...
        for msg in messages.data:
            if msg.role == "user":
                ...
                if not content:
                    logger.warning(
                        f"Empty or malformed user message in thread {thread_id}")
                    continue
...
async def detect_intent(description: str, thread_id: Optional[str] = None) -> Dict:
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"JSON parsing error in intent detection: {e}. Falling back to keyword method.")
            intents = ["medical"]
            symptoms = []
            symptom_keywords = [
        ...
        if not symptoms and "non_medical" not in intents:
            non_medical_keywords = [
                "what can you do", "hi", "hello", "how are you"]
        ...
        logger.error(
            f"Intent detection error: {e}. Using fallback keyword-based approach.")
        symptom_keywords = [
            "pain", "bleeding", "fever", "cough", "discomfort", "hurt", "ache", "nausea",
            "vomiting", "dizziness", "swelling", "shortness of breath", "palpitations"
        ]
    ...
    **Now: fully GPT-based extraction** (no regex dictionary). Send the text to GPT-4o-mini
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"GPT returned non-JSON or invalid format: {e}. No symptoms extracted.")
            unique_symptoms = []

        logger.info(
            f"Extracted symptoms (GPT): {unique_symptoms}, Duration: {duration}, Severity: {severity}")
...
async def generate_follow_up_questions(context: Dict) -> List[str]:
    ...
    try:
        ...
        symptoms_text = ", ".join(
            symptoms) if symptoms else "no symptoms reported yet"
        prompt = (
        ...
        try:
            ...
            logger.error(
                "JSON parsing error in follow-up questions. Using fallback questions.")
            questions = ["Do you have any other symptoms?",
...
def is_red_flag(full_text: str, severity: int = 0) -> bool:
    ...
    if severity >= 8 and any(term in lt for term in ["abdominal pain", "intermenstrual bleeding"]):
        logger.info(
            f"Red flag detected: high severity {severity} with critical symptom.")
        return True
    return False
...
async def should_query_pinecone_database(context: Dict) -> bool:
    ...
    if any(kw in full_text for kw in condition_keywords):
        logger.info(
            "User explicitly asked for condition identification → querying Pinecone.")
        return True

    logger.info(
        f"Not querying Pinecone: only {symptom_count} symptoms, no explicit condition request.")
    return False
...
async def query_index(query_text: str, symptoms: List[str], context: Dict, top_k: int = 50) -> List[Dict]:
    ...
    try:
        ...
        logger.info(
            f"Pinecone returned {len(matches)} raw matches for '{query_text[:50]}...'")
        ...
        logger.info(
            f"Selected {len(selected)} unique matches after thresholding.")
        return selected
...
async def generate_detailed_condition_description(condition_name: str, user_symptoms: List[str]) -> str:
    ...
    try:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if desc and len(desc) > 20:
                    logger.info(
                        f"Generated description for {condition_name}: {desc}")
                    return desc
                logger.warning(
                    f"Description too short for {condition_name}. Retrying...")
            except Exception as e:
                logger.error(
                    f"Error generating description for {condition_name} attempt {attempt+1}: {e}")
        ...
        logger.error(
            f"Fatal error generating description for {condition_name}: {e}")
    ...
    try:
        ...
        for match in matches:
            ...
            condition_data.append(
                {"name": disease, "description": desc, "score": score})
...
async def generate_phase2_response(context: Dict, is_emergency: bool, thread_id: Optional[str] = None) -> Dict:
    ...
    try:
        ...
        if should_query:
            logger.info(
                f"Querying Pinecone with {symptom_count} symptoms: {symptoms_text}")
            query_text = f"Symptoms: {symptoms_text}"
            matches = await query_index(query_text, symptoms, context)
            if not matches:
                logger.warning(
                    "No Pinecone matches found, using fallback condition.")
                possible_conditions = [
                    {
        ...
        if any("pain" in s.lower() for s in symptoms):
            self_care.append(
                "Apply a warm compress to painful areas for 10-15 minutes")
        if any("bleeding" in s.lower() for s in symptoms):
            self_care.append(
                "Monitor the bleeding and seek immediate care if it becomes heavy")
...
async def generate_contextual_response(description: str, thread_id: str, context: Dict) -> Dict:
    ...
    try:
        symptoms = context.get("all_symptoms", [])
        symptoms_text = ", ".join(
            symptoms) if symptoms else "your previously mentioned symptoms"
        symptom_count = len(symptoms)
        max_severity = context.get("max_severity", 0)
        is_emergency = is_red_flag(
            " ".join(context.get("user_messages", [])), max_severity)
        ...
        try:
            ...
            logger.error(
                f"Error adding contextual response to thread {thread_id}: {e}")
...
async def should_continue_conversation(thread_id: str, description: str) -> bool:
    ...
    try:
        ...
        if context["all_symptoms"] or context["assistant_responses"] > 0:
            logger.info(
                f"Continuing conversation in thread {thread_id} (prior symptoms/responses).")
            return True
...
async def triage(request: TriageRequest):
    ...
    try:
        ...
        logger.info(
            f"=== TRIAGE REQUEST === Description: '{description[:50]}...', Thread: {thread_id}")
        ...
        if "non_medical" in intents and not is_continuing and not context["all_symptoms"]:
            ...
            logger.info(
                f"Phase 2 (emergency={is_emergency}, should_query={should_query})")
            response = await generate_phase2_response(context, is_emergency, thread_id)
        else:
            logger.info(
                f"Phase 1 (assistant_responses={context['assistant_responses']+1}/{MAX_ITERATIONS})")
...
if __name__ == "__main__":
    ...
    # Verify that the index exists on startup
    try:
    ...
    logger.info(f"Starting triage server on port {PORT}...")
